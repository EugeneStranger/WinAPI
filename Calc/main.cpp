...
CONST CHAR* BUTTON_NAMES[] = { "point","plus","minus","aster","slash","equal","clear","BSP"};
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_CREATE:
	{
		...
		(
			...
			"Edit",
			"input",
			WS_CHILDWINDOW | WS_VISIBLE | ES_RIGHT | WS_BORDER,
			START_X, START_Y,
...
{
	CONST INT SIZE = 10;
	CHAR sz_filename[FILENAME_MAX] = {};//создаём и инициализируем имя файла значка
	for (int i = 0; i < SIZE; i++)
	{
		HWND hButton = GetDlgItem(hwnd, IDC_BUTTON_0 + i);//получаем окно кнопки
		sprintf(sz_filename, "ButtonsBMP\\%s\\button_%i.bmp",sz_skin, i);
		HBITMAP hBitmap = (HBITMAP)LoadImage//Загружаем картинку значка
		(
			GetModuleHandle(NULL),
			sz_filename,
			IMAGE_BITMAP,
		...
		SendMessage(hButton, BM_SETIMAGE, (WPARAM)IMAGE_BITMAP, (LPARAM)hBitmap);//Устанавливаем значок на кнопку
	}
	for (int i = 0; i < sizeof(BUTTON_NAMES) / sizeof(BUTTON_NAMES[0]); i++)
	{
		HWND hButton = GetDlgItem(hwnd, IDC_BUTTON_POINT + i);//получаем окно кнопки
		...
		(
			...
			sz_filename,
			IMAGE_BITMAP,
			BUTTON_SIZE, BUTTON_NAMES[i] == "equal" ? BUTTON_DOUBLE_SIZE : BUTTON_SIZE,
			LR_LOADFROMFILE
		);
		SendMessage(hButton, BM_SETIMAGE, (WPARAM)IMAGE_BITMAP, (LPARAM)hBitmap);//Устанавливаем значок на кнопку
	}
